...
; Case 2: Maria — 25yo grad student, lives in university housing.
; Parents pay tuition (~1/3 total support); Maria covers all other expenses via stipend.
; Hypothesis: No (QC: age 25 not < 24; QR: gross income $20,000 >> $5,000 exemption)

(assert (= candidate_age 25))
(assert (= taxpayer_age 55))
(assert (= gross_income 20000))    ; grad stipend covers all living expenses
(assert (= sheltered_workshop_income 0))
(assert (= exemption_amount 5000))
(assert (= candidate_self_support_percent 67))
(assert (= taxpayer_support_percent 33))
(assert (= taxpayer_agi 80000))
(assert (= competing_taxpayer_agi 0))
(assert (= months_same_abode 0))   ; lives in university housing, not with parents
(assert (= months_with_taxpayer 0))
...
(assert no_one_person_provided_over_half_support)
