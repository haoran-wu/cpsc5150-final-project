...
; Case 4: Linda — 45yo, lives with adult daughter (taxpayer), earns $5,000 from part-time work.
; Daughter provides more than half of Linda's total support.
; Exemption = $5,000 (from Project 4 base_152.smt2).
; Key: gross_income $5,000 is NOT strictly < exemption $5,000 → QR income test fails.
; QC: age 45 > taxpayer_age 30 → QC age comparison fails.
; Hypothesis: No (QR fails: $5,000 is not < $5,000; QC fails: Linda is older than daughter)

(assert (= candidate_age 45))
(assert (= taxpayer_age 30))    ; adult daughter is younger than Linda
(assert (= gross_income 5000))
(assert (= sheltered_workshop_income 0))
(assert (= exemption_amount 5000))
(assert (= candidate_self_support_percent 40))
(assert (= taxpayer_support_percent 60))
...
(assert is_parent)              ; Linda is the daughter's mother
...
(assert (not taxpayer_is_parent))   ; taxpayer (daughter) is NOT Linda's parent
