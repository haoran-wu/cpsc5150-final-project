...
; Case 3: James — 15yo, lives with parents during school year (~9 months),
; summers with grandparents (~3 months). Parents claim him, provide all support.
; Hypothesis: Yes (QC: child relationship, 9 months > 6, age 15 < 19, zero self-support)

(assert (= candidate_age 15))
(assert (= taxpayer_age 42))
...
(assert (= taxpayer_agi 70000))
(assert (= competing_taxpayer_agi 0))
(assert (= months_same_abode 9))   ; school year with parents
(assert (= months_with_taxpayer 9))
